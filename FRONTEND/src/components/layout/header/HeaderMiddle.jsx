...
const HeaderMiddle = () => {
  ...
  const mobileNavItems = [
    ...
    { label: "Electronics", icon: <ShoppingBag className="w-5 h-5" />, path: "/category/electronics" },
    { label: "Books", icon: <Package className="w-5 h-5" />, path: "/category/books" },
    { label: "Fashion", icon: <ShoppingBag className="w-5 h-5" />, path: "/category/fashion" },
    { label: "Furniture", icon: <Package className="w-5 h-5" />, path: "/category/furniture" },
    { label: "Hostels", icon: <Home className="w-5 h-5" />, path: "/category/hostels" },
    { label: "Store", icon: <Store className="w-5 h-5" />, path: "/store" },
  ];
  ...
  return (
    <div className={`bg-white border-b border-gray-100 sticky top-0 z-50 transition-all duration-300 ${scrolled ? 'shadow-md py-1' : 'shadow-sm'}`}>
      ...
      {/* Mobile Side Menu with enhanced design and animations */}
      {isMobileMenuOpen && (
          ...
          >
            <div className="p-6 border-b border-gray-100 bg-gradient-to-r from-teal-50 to-cyan-50">
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="p-2 rounded-full hover:bg-white/50 transition-all hover:scale-110 active:scale-95 group"
                >
                  <X className="w-5 h-5 text-gray-600 group-hover:rotate-90 transition-transform duration-300" />
                </button>
              </div>
              {isLoggedIn && (
                <div className="flex items-center gap-3 p-4 bg-white/80 backdrop-blur-sm rounded-xl hover:shadow-md transition-all duration-300 group">
                  <div className="relative">
                    <div className="w-12 h-12 rounded-full bg-gradient-to-br from-teal-100 to-cyan-100 flex items-center justify-center overflow-hidden ring-2 ring-white group-hover:ring-4 group-hover:ring-teal-100 transition-all">
                      {user?.avatar ? (
                        <img 
                          src={user.avatar} 
                          alt={user?.firstName || 'User'} 
            ...
            <div className="p-4 space-y-1">
              {mobileNavItems.map((item, idx) => (
                <button
                  ...
                  className="w-full text-left flex items-center gap-4 px-4 py-4 rounded-xl hover:bg-gradient-to-r hover:from-teal-50 hover:to-cyan-50 active:bg-gray-100 transition-all duration-200 group"
                  style={{ animation: `fadeInUp 0.3s ease-out ${idx * 0.05}s forwards`, opacity: 0 }}
                >
                  <div className="flex items-center justify-center w-10 h-10 rounded-lg bg-gray-100 text-gray-600 group-hover:bg-white group-hover:text-teal-600 group-hover:scale-110 transition-all duration-200 group-hover:shadow-md">
                    {item.icon}
                  </div>
                  <span className="font-medium text-gray-700 group-hover:text-teal-700 flex-1 text-left">
                    {item.label}
                  </span>
      ...
      {/* Add keyframe animations for mobile */}
      <style jsx>{`
        @keyframes fadeIn {
          from { opacity: 0; }
          to { opacity: 1; }

...
const AccountMenu = ({ isOpen, onClose, user, onLogout, accountMenuItems }) => {
  ...
  const handleTouchMove = (e) => {
    ...
    // Only allow swiping down when at the top of content
    if (diff > 0 && contentRef.current?.scrollTop === 0) {
      e.preventDefault();
      menuRef.current.style.transform = `translateY(${diff * 0.5}px)`;
      backdropRef.current.style.opacity = 1 - (diff * 0.002);
    }
  };
  ...
  const handleTouchEnd = (e) => {
    ...
    if (diff > 100) {
      // Swipe down threshold met - close menu
  ...
  const handleBackdropClick = (e) => {
    ...
    if (e.target === backdropRef.current || e.target.classList.contains('backdrop-blur-sm')) {
  ...
  // Prevent all clicks inside menu from bubbling
  ...
  return (
    <div className="lg:hidden">
      ...
      <div
        ref={backdropRef}
        className="fixed inset-0 bg-black/40 backdrop-blur-sm transition-opacity duration-300 z-[9]"
        style={{ 
          opacity: 1,
          pointerEvents: 'auto'
        }}
      ...
      >
        ...
        <div 
          className="w-full flex justify-center pt-3 pb-1 cursor-grab active:cursor-grabbing touch-pan-y"
          onTouchStart={handleTouchStart}
          onTouchMove={handleTouchMove}
          onTouchEnd={handleTouchEnd}
        >
